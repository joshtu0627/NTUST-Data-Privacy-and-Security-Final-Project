...
def partition(df, feature_columns,sensitive_column,k):
    finished_partitions=[]
    partitions=[df.index]
    i=0
    while partitions:
        ...
        for column in feature_columns:
            ...
            if is_k_anonymous(l,k) and is_k_anonymous(r,k):
...
def build_data(df,k):
    ...
    finished_partitions = partition(df, feature_columns, sensitive_column,k)
...
# 儲存模型
model.save('model/k-Anonymity_data.h5')