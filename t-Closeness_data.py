# 這個程式的目標是用神經網路來分析經過t-closeness後的原始資料
...
def partition(df, feature_columns,sensitive_column,k,global_freqs):
    finished_partitions=[]
    partitions=[df.index]
    i=0
    while partitions:
        partition=partitions.pop()
        for column in feature_columns:# age , education, sex, occupation 
            l,r=split(df,partition,column)
            if is_k_anonymous(l,k) and is_k_anonymous(r,k) and is_t_close(df,l,sensitive_column,global_freqs,p=0.2) and is_t_close(df,r,sensitive_column,global_freqs,p=0.2):
...
def t_closeness(df, partition, column, global_freqs):
    ...
    for value, count in group_counts.to_dict().items():
        for i in count.keys():
            ...
            d = abs(p-global_freqs[i])
            if d_max is None or d > d_max:
                d_max = d
    return d_max

def is_t_close(df, partition, sensitive_column, global_freqs, p=1.0):
...
def build_data(df,k):
    ...
    for value, count in group_counts.to_dict().items():
        ...
        for i in list_key:
            p = count[i]/total_count
            global_freqs[i] = p
    finished_partitions = partition(df, feature_columns, sensitive_column,k,global_freqs)
...
# 儲存模型
model.save('model/t-Closeness_data.h5')